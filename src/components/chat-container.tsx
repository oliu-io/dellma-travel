...
import { seedInitialChat, dispatchChatAction } from "@/lib/chat-flow";
import { BlockRenderer } from "./chat/block-renderer";
import { Button } from "@/components/ui/button";
import { Bike, RotateCcw, Loader2 } from "lucide-react";

const STAGE_ORDER = ["setup", "forecast", "preferences", "decision", "challenge"] as const;
...
export function ChatContainer() {
  ...
  // Determine locked stage index — blocks in stages before the current one are locked
  ...
  chatBlocks.forEach((block, idx) => {
    if (block.kind === "stage-divider") {
      ...
      // Map title back to stage key
      const stageKey = Object.entries(stageLabels).find(
  ...
  function isBlockLocked(blockIndex: number): boolean {
    // Find the stage this block belongs to
    let blockStage = "setup";
    for (const sd of stageDividerIndices) {
      if (blockIndex >= sd.idx) blockStage = sd.stage;
    }
    const blockStageIdx = STAGE_ORDER.indexOf(blockStage as typeof STAGE_ORDER[number]);
        ...
        <div className="max-w-3xl mx-auto flex items-center justify-between px-6 py-3">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-primary/10 flex items-center justify-center">
              <Bike className="w-5 h-5 text-primary" strokeWidth={1.8} />
            </div>
            <div>
